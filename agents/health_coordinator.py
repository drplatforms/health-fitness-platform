...
def generate_health_report(user_id):
    ...
    recovery_task = Task(
        description=f"""
        Analyze the following recovery data.
    ...
    nutrition_task = Task(
        description=f"""
        Analyze the following nutrition data.
    ...
    print(
        f"Start Time: "
        f"{start_timestamp.strftime('%Y-%m-%d %I:%M:%S %p')}"
    )
    ...
    try:
        ...
        print(
            f"End Time: "
            f"{end_timestamp.strftime('%Y-%m-%d %I:%M:%S %p')}"
        )

        print(
            f"Runtime: "
            f"{runtime_seconds} seconds "
            f"({runtime_minutes} minutes)"
        )
        ...
        save_health_report(
            ...
            model_summary="All 8B Benchmark"
        )
