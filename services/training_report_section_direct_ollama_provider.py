# ruff: noqa: E402
"""Formal direct Ollama provider implementation for training report sections.

The legacy script path imports this module as a CLI/runtime-QA compatibility
wrapper. Keep behavior-compatible with the accepted spike contract while making
service ownership explicit for provider-boundary integration.
"""
...
def run_direct_ollama_training_report_section_provider(
    *args, **kwargs
) -> DirectOllamaTrainingReportSectionSpikeResult:
    """Run the formal direct Ollama training report section provider.

    This function is the provider-owned entry point. The legacy spike wrapper
    calls this same implementation so runtime QA scripts keep working without
    treating the script module as the backend service owner.
    """

    return run_direct_ollama_training_report_section_spike(*args, **kwargs)
