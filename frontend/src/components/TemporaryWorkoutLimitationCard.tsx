...
export function TemporaryWorkoutLimitationCard({
  ...
  return (
    <section className="min-w-0 rounded-xl bg-surface-subtle px-3 py-2.5 ring-1 ring-border">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-text-strong">
            {state?.active ? "Temporary limitation active" : "Temporary limitation"}
          </p>
          {state?.limitation ? (
        ...
        <div className="flex gap-2">
          <button
            ...
            className="rounded-lg bg-surface px-3 py-1.5 text-xs font-semibold text-text-body ring-1 ring-border hover:bg-surface-highlighted"
          >
            {state?.active ? "Edit" : "Set"}
          </button>
          {state?.active ? (
            <button
              type="button"
              ...
              disabled={isSaving}
              className="rounded-lg px-3 py-1.5 text-xs font-semibold text-text-body hover:bg-surface-highlighted disabled:opacity-50"
            >
              Clear
            </button>
          ) : null}
      ...
      {state?.current_plan_conflicts.length ? (
        <p role="status" className="mt-2 rounded-lg bg-warning-surface px-2.5 py-2 text-xs font-medium text-warning-foreground">
          This workout includes {state.current_plan_conflicts.length} exercise
          {state.current_plan_conflicts.length === 1 ? "" : "s"} that {state.current_plan_conflicts.length === 1 ? "is" : "are"} now temporarily restricted.
        </p>
      ) : null}
      ...
      {isEditing ? (
        <div className="mt-3 space-y-4 border-t border-border-subtle pt-3">
          <p className="text-xs text-text-body">
            Only the movements and exercises you select are blocked.
          </p>

          <fieldset>
            <legend className="text-xs font-semibold text-text-strong">Affected area (optional)</legend>
            <div className="mt-2 flex flex-wrap gap-2">
              {AFFECTED_REGION_OPTIONS.map((region) => (
                <label key={region} className="flex items-center gap-1.5 rounded-lg bg-surface px-2 py-1.5 text-xs text-text-body ring-1 ring-border">
                  <input
                    type="checkbox"
                    checked={affectedRegions.includes(region)}
                    onChange={() => setAffectedRegions(toggleValue(affectedRegions, region))}
                  />
          ...
          <fieldset>
            <legend className="text-xs font-semibold text-text-strong">Avoid movements</legend>
            <div className="mt-2 flex max-h-32 flex-wrap gap-2 overflow-y-auto p-0.5">
              {movementOptions.map((movement) => (
                <label key={movement} className="flex items-center gap-1.5 rounded-lg bg-surface px-2 py-1.5 text-xs text-text-body ring-1 ring-border">
                  <input
                    type="checkbox"
                    checked={restrictedMovements.includes(movement)}
                    onChange={() => setRestrictedMovements(toggleValue(restrictedMovements, movement))}
                  />
          ...
          <div>
            <label htmlFor="temporary-limitation-exercise-search" className="text-xs font-semibold text-text-strong">
            ...
            {searchResults.length ? (
              <div className="mt-2 grid gap-1" role="list" aria-label="Exercise search results">
                {searchResults.map((exercise) => (
                  <button
                    ...
                    className="rounded-lg bg-surface px-3 py-2 text-left text-sm text-text-body ring-1 ring-border hover:bg-surface-highlighted"
                  >
                    {exercise.name}
                  </button>
                ))}
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {selectedExercises.map((exercise) => (
                  <button
                    ...
                    onClick={() => setExcludedIds(excludedIds.filter((id) => id !== exercise.id))}
                    className="rounded-full bg-surface px-2.5 py-1 text-xs text-text-body ring-1 ring-border"
                  >
                    {exercise.name} ×
                  </button>
                ))}
          ...
          <label className="block text-xs font-semibold text-text-strong">
            ...
            >
              {state?.limitation?.expires_at ? <option value="existing">Keep current end date</option> : null}
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              disabled={isSaving || (!restrictedMovements.length && !excludedIds.length)}
              className="rounded-lg bg-action-primary px-3 py-2 text-sm font-semibold text-action-primary-foreground disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={isSaving}
              className="rounded-lg px-3 py-2 text-sm font-semibold text-text-body hover:bg-surface-highlighted"
            >
              Cancel
            </button>
            {state?.active ? (
      ...
      {error ? <p role="alert" className="mt-2 text-xs font-medium text-danger-foreground">{error}</p> : null}
    </section>
  );
}
