...
test("builds the six primary daily workspace routes", () => {
  ...
  assert.equal(buildDailyWorkspaceHref("coach", 101), "/coach?user_id=101");
  assert.equal(
...
test("maps nested personal-food routes to the Food destination", () => {
  ...
  assert.equal(activeDailyWorkspace("/coach"), "coach");
