import { NextRequest, NextResponse } from "next/server";

import { getApiBaseUrl } from "@/lib/dailyDriverApi";

export async function POST(request: NextRequest) {
  const userId = request.nextUrl.searchParams.get("user_id");
  const targetDate = request.nextUrl.searchParams.get("target_date");
  if (!userId || !targetDate) {
    return NextResponse.json(
      { detail: "user_id and target_date are required." },
      { status: 400 },
    );
  }

  const body = await request.json().catch(() => null);
  if (!body || typeof body !== "object") {
    return NextResponse.json({ detail: "A JSON request is required." }, { status: 400 });
  }

  try {
    const response = await fetch(
      `${getApiBaseUrl()}/nutrition/${userId}/meal-ideas?target_date=${encodeURIComponent(targetDate)}`,
      {
        method: "POST",
        cache: "no-store",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify(body),
      },
    );
    const payload = await response.json().catch(() => null);
    return NextResponse.json(
      payload ?? { detail: "The backend returned an empty meal-ideas response." },
      { status: response.status },
    );
  } catch {
    return NextResponse.json(
      { detail: "Unable to reach the meal ideas endpoint." },
      { status: 502 },
    );
  }
}
